...
IMPLEMENT_DYNCREATE(CCscopeView, CView)

CCscopeView::CCscopeView()
{
	g_pCscope = this;
...
BEGIN_MESSAGE_MAP(CCscopeView, CView)
	ON_WM_TIMER()
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_SIZING()
...
void CCscopeView::OnDraw(CDC* pDC)
{
	CDocument* pDoc = GetDocument();
	pDoc->SetTitle(_T("C-scan Analysis"));
}

// CCscopeView message handlers

int CCscopeView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CCscopeView::OnTimer(UINT_PTR nIDEvent)
{
...
void CCscopeView::OnSize(UINT nType, int cx, int cy)
{
...
void CCscopeView::OnSizing(UINT fwSide, LPRECT pRect)
{
	CView::OnSizing(fwSide, pRect);
	m_bOnSizing = true;
}

void CCscopeView::OnInitialUpdate()
{
	CView::OnInitialUpdate();
}

void CCscopeView::UpdateDisplayData()
{
	if (g_uiMode == 1)
	{
		ResetBuffer();
...
void CCscopeView::SetEvent(UINT Event)
{
	m_uiEvent = Event;
...
void CCscopeView::CreateAngleTable()
{
	if (m_pCosTable[0])
		delete[] m_pCosTable[0];
...
void CCscopeView::CreatePalette()
{
...
void CCscopeView::ResetBuffer()
{
	...
	m_iBitmapHeight = (int)min(m_iViewHeight, (int)GetAnalysisEncoderPositionQuantity());
	...
	m_pPositionProbe_Z[0] = new double[InfoProbe1Analysis.iCycleCount * InfoProbe1Analysis.PointQuantity];

	m_pPositionProbe_X[1] = new int[InfoProbe2Analysis.iCycleCount * InfoProbe2Analysis.PointQuantity];
	m_pPositionProbe_Z[1] = new double[InfoProbe2Analysis.iCycleCount * InfoProbe2Analysis.PointQuantity];

	memset(m_pPositionProbe_X[0], -1, InfoProbe1Analysis.iCycleCount * InfoProbe1Analysis.PointQuantity * sizeof(int));
	memset(m_pPositionProbe_Z[0], -1, InfoProbe1Analysis.iCycleCount * InfoProbe1Analysis.PointQuantity * sizeof(double));

	memset(m_pPositionProbe_X[1], -1, InfoProbe2Analysis.iCycleCount * InfoProbe2Analysis.PointQuantity * sizeof(int));
	memset(m_pPositionProbe_Z[1], -1, InfoProbe2Analysis.iCycleCount * InfoProbe2Analysis.PointQuantity * sizeof(double));
}

void CCscopeView::SetMapping()
{
	...
	if (g_uiProbeAnalysis == 0)
	{
		for (unsigned int i = 0 ; i < (unsigned int)InfoProbe1Analysis.iCycleCount ; i++)
		{
			for (unsigned int j = 0 ; j < (unsigned int)InfoProbe1Analysis.PointQuantity ; j++)
			{
				...
				m_pPositionProbe_Z[0][i * InfoProbe1Analysis.PointQuantity + j] = Lh;
			}
		}
	}
	else if (g_uiProbeAnalysis == 1)
	{
		for (unsigned int i = 0 ; i < (unsigned int)InfoProbe2Analysis.iCycleCount ; i++)
		{
			for (unsigned int j = 0 ; j < (unsigned int)InfoProbe2Analysis.PointQuantity ; j++)
			{
				...
				m_pPositionProbe_Z[1][i * InfoProbe2Analysis.PointQuantity + j] = Lh;
			}
		}
	}
	else if (g_uiProbeAnalysis == 2)
...
void CCscopeView::DisplayProbe1Gate()
{
	double XMin = GetAnalysisXAxisMin();
	double XMax = GetAnalysisXAxisMax();
	...
	int IndexYMin = (int)(m_iBitmapHeight * (MinimumY_Index) / GetAnalysisEncoderPositionQuantity());
	int IndexYMax = (int)(m_iBitmapHeight * (MaximumY_Index) / GetAnalysisEncoderPositionQuantity());
	...
	for (unsigned int h = (unsigned int)IndexYMin ; h <= (unsigned int)IndexYMax ; h++)
	{
		for (unsigned int i = 0 ; i < (unsigned int)InfoProbe1Analysis.iCycleCount ; i++)
		{
			for (unsigned int j = 0 ; j < (unsigned int)InfoProbe1Analysis.PointQuantity ; j++)
			{
				...
				if ((m_pPositionProbe_Z[0][TempPosition] >= Analysis_ZMIN) &&
					(m_pPositionProbe_Z[0][TempPosition] <= Analysis_ZMAX) &&
					(m_pPositionProbe_X[0][TempPosition] >= IndexXMin) &&
					(m_pPositionProbe_X[0][TempPosition] <= IndexXMax))
				{
					int Position = h * m_iBitmapWidth + m_pPositionProbe_X[0][TempPosition];

					if (m_pPositionProbe_X[0][TempPosition] < m_iBitmapWidth)
					{
...
void CCscopeView::DisplayProbe2Gate()
{
	double XMin = GetAnalysisXAxisMin();
	double XMax = GetAnalysisXAxisMax();
	double YMin = GetAnalysisYAxisMin();
	double YMax = GetAnalysisYAxisMax();
	...
	for (unsigned int h = (unsigned int)IndexYMin ; h <= (unsigned int)IndexYMax ; h++)
	{
		for (unsigned int i = 0 ; i < (unsigned int)InfoProbe2Analysis.iCycleCount ; i++)
		{
			for (unsigned int j = 0 ; j < (unsigned int)InfoProbe2Analysis.PointQuantity ; j++)
			{
				...
				if ((m_pPositionProbe_Z[1][TempPosition] >= Analysis_ZMIN) &&
					(m_pPositionProbe_Z[1][TempPosition] <= Analysis_ZMAX) &&
					(m_pPositionProbe_X[1][TempPosition] >= IndexXMin) &&
					(m_pPositionProbe_X[1][TempPosition] <= IndexXMax))
				{
					int Position = h * m_iBitmapWidth + m_pPositionProbe_X[1][TempPosition];

					if (m_pPositionProbe_X[1][TempPosition] < m_iBitmapWidth)
					{
...
void CCscopeView::OnLButtonUp(UINT nFlags, CPoint point)
{
