...
void CDlgCalibration::UpdateDialog()
{
	...
	if(bDone)
		...
		m_bCheckEnable = FALSE;
	m_CheckOffset.EnableWindow(bDone);
	m_CheckOffset.ShowWindow(m_pHWDeviceOEMPA->GetSWDeviceOEMPA()->IsTimeOffsetSupported()?true:false);
	if(bDone)
	{
		m_strStaticAlignment.Format(L"(%.0f ns)",m_pHWDeviceOEMPA->GetSWDeviceOEMPA()->GetCalibrationAlignment()*1e9f);
		m_strStaticOffset.Format(L"(%.0f ns)",m_pHWDeviceOEMPA->GetSWDeviceOEMPA()->GetCalibrationOffset()*1e9f);
		if(bEnable)
			m_bCheckEnable = TRUE;
		else
			m_bCheckEnable = FALSE;
		if(m_pHWDeviceOEMPA->GetSWDeviceOEMPA()->GetCalibrationOffset()==m_pHWDeviceOEMPA->GetSWDeviceOEMPA()->GetTimeOffset())
			m_bCheckOffset = TRUE;
		else
			m_bCheckOffset = FALSE;
	}
...
void CDlgCalibration::OnBnClickedCheckCalibrationOffset()
{
	...
	if(m_pHWDeviceOEMPA->LockDevice())
	{
		if(m_bCheckOffset)
		{
			fCalibrationOffset = m_pHWDeviceOEMPA->GetSWDeviceOEMPA()->GetCalibrationOffset();
...
void CDlgCalibration::OnBnClickedButtonCalibrationPerform()
{
	...
	CString string;
	bool bReportFile;
	float fCorrectionMax,fCalibrationOffset;
	...
	if(!m_pHWDeviceOEMPA->PerformCalibration(fCorrectionMax,fCalibrationOffset))
	{
		if(bReportFile)
	...
	dwStop = GetTickCount();
	string.Format(L"AlignmentDelayMax = %.0f ns\r\nCorrectionOffset = %.0f ns\r\nCalibrationTime = %d ms",fCorrectionMax*1e9f,fCalibrationOffset*1e9f,(int)(dwStop-dwStart));
	m_strStaticAlignment.Format(L"(%.0f ns)",fCorrectionMax*1e9f);
	m_strStaticOffset.Format(L"(%.0f ns)",fCalibrationOffset*1e9f);
