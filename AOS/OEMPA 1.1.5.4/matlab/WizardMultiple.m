...
SetupFileName = 'C:\Users\Public\Share\matrix\ExampleMultiple.txt';
WizardFileName = 'C:\Users\Public\Share\matrix\WizardMultiple.txt';
...
if(mxToolboxIsRunning())
    ...
    if (0 && (exist(WizardFileName, 'file') == 2))
        ...
        mxSetToolboxParameter('OEMPA_channel_0\Channels\Probe:100\Scan.count=2');
        
        for iScan=100:101
            v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\Wave=Transverse');
            mxSetToolboxParameter(v);
            v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\DepthMode=SoundPath');
            mxSetToolboxParameter(v);

            v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStart\Aperture\Index.Count=128');
            mxSetToolboxParameter(v);
            for ii=0:127
                v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStart\Aperture\Index:',int2str(ii),'=',int2str(ii))
                mxSetToolboxParameter(v);
            end
            v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStart\EmissionDepth=25.4 mm');
            mxSetToolboxParameter(v);
            v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStart\Depth.count=1');
            mxSetToolboxParameter(v);
            v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStart\Depth:0=25.4 mm');
            mxSetToolboxParameter(v);
            if(iScan==100)
                v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStart\Angle=30.0 deg');
            else
                v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStart\Angle=40.0 deg');
            end
            mxSetToolboxParameter(v);
            v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStart\Skew=90.0 deg');
            mxSetToolboxParameter(v);

            for ii=0:127
                v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStop\Aperture\Index:',int2str(ii),'=',int2str(ii));
                mxSetToolboxParameter(v);
            end
            v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStop\Depth.count=1');
            mxSetToolboxParameter(v);
            v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStop\Depth:0=25.4 mm');
            mxSetToolboxParameter(v);
            if(iScan==100)
                v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStop\Angle=39.0 deg');
            else
                v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStop\Angle=50.0 deg');
            end
            mxSetToolboxParameter(v);
            v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStop\Skew=90.0 deg');
            mxSetToolboxParameter(v);
        
            for ii=0:127
                v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStep\Aperture\Index:',int2str(ii),'=',int2str(ii));
                mxSetToolboxParameter(v);
            end
            v = strcat('OEMPA_channel_0\Channels\Probe:100\Scan:',int2str(iScan),'\WizardScan\VectorStep\Angle=1.0 deg');
            mxSetToolboxParameter(v);
        end
