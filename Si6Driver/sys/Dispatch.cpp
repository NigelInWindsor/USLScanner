...
/////////////////////////////////////////////////////////////////////////////
//	SI6Create			Handle Create/Open file IRP
...
//	SI6Close			Handle Close file IRPs
//	SI6Read			Handle Read IRPs
//	SI6Write			Handle Write IRPs
//	SI6DeviceControl	Handle DeviceIoControl IRPs
//	SI6SystemControl	Handle WMI IRPs
/////////////////////////////////////////////////////////////////////////////
...
NTSTATUS GetResourcesFromFilename( IN PUNICODE_STRING usfilename, IN PSI6_DEVICE_EXTENSION dx);
...
NTSTATUS ClaimResources( IN PDEVICE_OBJECT si6ddo);
NTSTATUS TranslateAndMapResources( IN PDEVICE_OBJECT si6ddo);
void UnclaimResources( IN PDEVICE_OBJECT si6ddo);
void FreeUserStruct( IN PSI6_DEVICE_EXTENSION dx);
void FreeTimeSlot( IN PSI6_DEVICE_EXTENSION dx);
void FreeDataArray( IN PSI6_DEVICE_EXTENSION dx);

NTSTATUS PowerUpDevice( IN PDEVICE_OBJECT si6ddo);
						...
						IN PIRP Irp)
{
	...
	status = TranslateAndMapResources(SI6ddo);
	if( !NT_SUCCESS(status))
	{
		UnclaimResources(SI6ddo);
		goto fail;
	}
	//USL INITIALIZATION
	nTable[0]=0xffff;
...
NTSTATUS ClaimResources( IN PDEVICE_OBJECT SI6ddo)
{
	...
	DebugPrint("SI6ddo->DriverObject %x",SI6ddo->DriverObject);
	if( dx->ResourceOverride) {
		DebugPrintMsg("Resource override conflict");
	}
	BOOLEAN ConflictDetected;
	NTSTATUS status = IoReportResourceUsage( NULL,
						SI6ddo->DriverObject, ResourceList, ListSize,	// Driver resources
...
void UnclaimResources( IN PDEVICE_OBJECT SI6ddo)
{
	...
	IoReportResourceUsage( NULL,
					SI6ddo->DriverObject, &ResourceList, sizeof(ResourceList),	// Driver resources
					NULL, NULL, 0,	// Device resources
...
void FreeResources( IN PDEVICE_OBJECT SI6ddo)
{
	PSI6_DEVICE_EXTENSION dx = (PSI6_DEVICE_EXTENSION)SI6ddo->DeviceExtension;
					...
					IN PIRP Irp)
{
	...
	else
	{
		...
		IoStartPacket( si6ddo, Irp, 0, SI6CancelIrp);
...
//		DebugPrintMsg("DeviceIoControl: StartIo called");
		...
		return STATUS_PENDING;
		break;
	case IOCTL_SI6_GET_VERSION:
		if( (sizeof Version)>OutputLength)
			status = STATUS_INVALID_PARAMETER;
		else
		{
			strcpy(Version,SI6_VERSION);
			BytesTxd = sizeof Version;
			RtlCopyMemory(Irp->AssociatedIrp.SystemBuffer,Version,BytesTxd);
		}
		break;
/*
	case IOCTL_SI6_GET_PTR_SI6:
		if( (sizeof (struct SI6Data))>OutputLength)
			status = STATUS_INVALID_PARAMETER;
		else
		{
		...
		break;
	case IOCTL_SI6_DRIVER_SI6STRUCT_TO_USER:
		...
		break;
	case IOCTL_SI6_USER_SI6STRUCT_TO_DRIVER:
		if( (sizeof si6)<InputLength)
			status = STATUS_INVALID_PARAMETER;
		else
		{
			RtlCopyMemory(&si6,Irp->AssociatedIrp.SystemBuffer,InputLength);
		}
		break;
	case IOCTL_SI6_SEND_PTR_USER_SI6:
		if(InputLength!=0)
			status = STATUS_INVALID_PARAMETER;
		else
		{
//			psi6=(struct SI6Data *)IrpStack->Parameters.DeviceIoControl.Type3InputBuffer;
//			psi6->nEcho=0x123;
		}
		break;
*/
	case IOCTL_SI6_SHARE_USER_STRUCT:
		if(InputLength!=0)
			status = STATUS_INVALID_PARAMETER;
		else
		{
			FreeUserStruct(dx);
			dx->pVirtualUserStruct=(struct SI6Data *)IrpStack->Parameters.DeviceIoControl.Type3InputBuffer;
			DebugPrint("Virtual Address = %x",dx->pVirtualUserStruct);
			
			dx->mdl = IoAllocateMdl(dx->pVirtualUserStruct,sizeof (struct SI6Data),FALSE,FALSE,NULL);
			if(dx->mdl) {
				MmProbeAndLockPages(dx->mdl,KernelMode,IoModifyAccess);
				dx->pSi6User=(struct SI6Data *)MmGetSystemAddressForMdl(dx->mdl);
				DebugPrint("system Addrs = %x",dx->pSi6User);
				int ii;
				for(ii=0;ii<6;ii++) {
					DebugPrint("Axis Addrs %d = %x",ii+1,dx->pSi6User->pAxis[ii]);
					dx->mdlAxis[ii] = IoAllocateMdl(dx->pSi6User->pAxis[ii],sizeof (struct AxisData),FALSE,FALSE,NULL);
					if(dx->mdlAxis[ii]) {
		...
		break;
	case IOCTL_SI6_FREE_USER_STRUCT:	FreeUserStruct(dx);
		break;
	case IOCTL_SI6_SHARE_DATA_ARRAY:
		if((InputLength!=0)||(dx->pSi6User->pData==NULL)) {
			status = STATUS_INVALID_PARAMETER;
		}
		else
		{
			FreeDataArray(dx);
			DebugPrint("Virtual Data Array Address = %x",dx->pSi6User->pData);
			dx->mdlDataArray = IoAllocateMdl(dx->pSi6User->pData,dx->pSi6User->nDataSize,FALSE,FALSE,NULL);
			if(dx->mdlDataArray!=NULL) {
				...
				DebugPrint("system Addrs = %x %d",dx->pDataArray,dx->pSi6User->nDataSize);
			};
		};
		break;
	case IOCTL_SI6_FREE_DATA_ARRAY:	FreeDataArray(dx);
		break;
	case IOCTL_SI6_SHARE_TIMESLOT_STRUCT:
		if(InputLength!=0) {
			status = STATUS_INVALID_PARAMETER;
		}
		else
		...
		break;
	case IOCTL_SI6_FREE_TIMESLOT_STRUCT:	FreeTimeSlot(dx);
		break;
	case IOCTL_SI6_READ_ADC200_MEMORY:
		if(OutputLength<=0)
			status = STATUS_INVALID_PARAMETER;
		else
		{
