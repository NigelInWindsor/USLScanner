...
void CResizableLayout::AddAnchor(HWND hWnd, CSize sizeTypeTL, CSize sizeTypeBR,CSize sizeTypeOffset,BOOL LockTL,BOOL LockBR)
{
	...
	if(LockTL == FALSE) {
		sizeMarginTL.cx = rectChild.left - rectParent.Width() * sizeTypeTL.cx / 100 + sizeTypeOffset.cx;
		sizeMarginTL.cy = rectChild.top - rectParent.Height() * sizeTypeTL.cy / 100 + sizeTypeOffset.cy;
	} else {
		sizeMarginTL.cx = rectChild.left;
		sizeMarginTL.cy = rectChild.top;
	}
	...
	if(LockBR == FALSE) {
		...
		sizeMarginBR.cx = rectChild.right;
		sizeMarginBR.cy = rectChild.bottom;
	}

	// add to the list
	LayoutInfo layout(hWnd, sizeTypeTL, sizeMarginTL,
		sizeTypeBR, sizeMarginBR, bHScroll, bRefresh,LockTL, LockBR);
...
void CResizableLayout::ArrangeLayout()
{
	...
	for (i=0; i<count; ++i)
	{
		...
		if(layout.bLockTL == FALSE) {
			...
			newrc.left = layout.sizeMarginTL.cx;
			newrc.top = layout.sizeMarginTL.cy;
		}
	
		// calculate new bottom-right corner

		if(layout.bLockBR == FALSE) {
			...
			newrc.right = layout.sizeMarginBR.cx;
			newrc.bottom = layout.sizeMarginBR.cy;
		}
