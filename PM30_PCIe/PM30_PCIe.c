  ...
  )
{
	...
	KdPrint (("PM30_PCIe.sys: DriverEntry\n"));
	...
	KdPrint (("PM30_PCIe.sys: DriverEntry complete\n"));
  ...
  )
{
	...
	case IOCTL_INITIATOR_RESET:
		...
		KdPrint(("PM30_PCIe.sys: IOCTL_INITIATOR_RESET\n"));
	...
	case IOCTL_CLEAR_INITIATOR_RESET:
		...
		KdPrint(("PM30_PCIe.sys: IOCTL_CLEAR_INITIATOR_RESET\n"));
		address = (ULONG) deviceExtension->MemoryStart[FPGA] + DCR_OFFSET;
		length = 1;
	...
	case IOCTL_SET_DMA_WRITE_ADDR:
		...
		KdPrint(("PM30_PCIe.sys: IOCTL_SET_DMA_WRITE_ADDR"));
	...
	case IOCTL_SET_DMA_WRITE_SIZE:
		...
		KdPrint(("PM30_PCIe.sys: IOCTL_SET_DMA_WRITE_SIZE"));
	...
	case IOCTL_SET_DMA_WRITE_COUNT:
		...
		WRITE_REGISTER_ULONG ((PULONG) address, *(pBuffer));	// Write the new register value
		KdPrint(("PM30_PCIe.sys: IOCTL_SET_DMA_WRITE_COUNT"));
	...
	case IOCTL_SET_DMA_WRITE_PATTERN:
		...
		KdPrint(("PM30_PCIe.sys: IOCTL_SET_DMA_WRITE_PATTERN"));
	...
	case IOCTL_SET_DMA_READ_ADDR:
		...
		// Set the DMA target address, ensuring that the value is aligned on a 4-byte boundary.
		WRITE_REGISTER_ULONG ((PULONG) address, *(pBuffer) & 0xFFFFFFFC);
		KdPrint(("PM30_PCIe.sys: IOCTL_SET_DMA_READ_ADDR"));
	...
	case IOCTL_SET_DMA_READ_SIZE:
		...
		KdPrint(("PM30_PCIe.sys: IOCTL_SET_DMA_READ_SIZE"));
	...
	case IOCTL_SET_DMA_READ_COUNT:
		...
		regValue |= *(pBuffer) & 0xFFFF;						// Set the new count
		WRITE_REGISTER_ULONG ((PULONG) address, regValue);		// Write the new register value
		KdPrint(("PM30_PCIe.sys: IOCTL_SET_DMA_READ_COUNT"));
	...
	case IOCTL_SET_DMA_READ_PATTERN:
		...
		length = 1;
		WRITE_REGISTER_ULONG ((PULONG) address, *(pBuffer));	// Write the new register value from pBuffer
		KdPrint(("PM30_PCIe.sys: IOCTL_SET_DMA_READ_PATTERN"));
	...
	case IOCTL_DMA_START:
		...
		KdPrint(("PM30_PCIe.sys: IOCTL_DMA_START"));
	...
	case IOCTL_GET_DMA_STATUS:
		...
		*pBuffer = READ_REGISTER_ULONG ((PULONG) address);
		KdPrint(("PM30_PCIe.sys: IOCTL_GET_DMA_STATUS"));
	...
	case IOCTL_GET_DMA_WRITE_PERF:
		...
		length = 1;
		*pBuffer = READ_REGISTER_ULONG ((PULONG) address);
		KdPrint(("PM30_PCIe.sys: IOCTL_GET_DMA_WRITE_PERF"));
	...
	case IOCTL_GET_DMA_READ_PERF:
		...
		length = 1;
		*pBuffer = READ_REGISTER_ULONG ((PULONG) address);
		KdPrint(("PM30_PCIe.sys: IOCTL_GET_DMA_READ_PERF"));
	...
	case IOCTL_GET_DMA_WRITE_ADDR:
		...
		*pBuffer = READ_REGISTER_ULONG ((PULONG) address) & 0xFFFFFFFC;		// 4-byte boundary
		KdPrint(("PM30_PCIe.sys: IOCTL_GET_DMA_WRITE_ADDR"));
		KdPrint(("   dma write addr = 0x%x\n", *(pBuffer) ));
		break;
	...
	case IOCTL_GET_DMA_WRITE_SIZE:
		...
		*pBuffer = READ_REGISTER_ULONG ((PULONG) address) & 0xFFF;			// Lower 12 bits only
		KdPrint(("PM30_PCIe.sys: IOCTL_GET_DMA_WRITE_SIZE"));
		KdPrint(("   dma write size = %d\n", *(pBuffer)));
		break;
	...
	case IOCTL_GET_DMA_WRITE_COUNT:
		...
		*pBuffer = READ_REGISTER_ULONG ((PULONG) address) & 0xFFFF;			// Lower 16 bits only
		KdPrint(("PM30_PCIe.sys: IOCTL_GET_DMA_WRITE_COUNT"));
		KdPrint(("   dma write count = %d\n", *(pBuffer)));
		break;
	...
	case IOCTL_GET_DMA_WRITE_PATTERN:
		...
		length = 1;
		*pBuffer = READ_REGISTER_ULONG ((PULONG) address);	
		KdPrint(("PM30_PCIe.sys: IOCTL_GET_DMA_WRITE_PATTERN"));
		KdPrint(("   dma write pattern = 0x%x\n", *(pBuffer)));	
		break;
	...
	case IOCTL_GET_DMA_READ_ADDR:
		...
		length = 1;
		*pBuffer = READ_REGISTER_ULONG ((PULONG) address) & 0xFFFFFFFC;		// 4-byte boundary
		KdPrint(("PM30_PCIe.sys: IOCTL_GET_DMA_READ_ADDR"));
		KdPrint(("   dma read addr = 0x%x\n", *(pBuffer) ));
		break;
	...
	case IOCTL_GET_DMA_READ_SIZE:
		...
		length = 1;
		*pBuffer = READ_REGISTER_ULONG ((PULONG) address) & 0xFFF;			// Lower 12 bits only
		KdPrint(("PM30_PCIe.sys: IOCTL_GET_DMA_READ_SIZE"));
		KdPrint(("   dma read size = %d\n", *(pBuffer)));
		break;
	...
	case IOCTL_GET_DMA_READ_COUNT:
		...
		length = 1;
		*pBuffer = READ_REGISTER_ULONG ((PULONG) address) & 0xFFFF;			// Lower 16 bits only
		KdPrint(("PM30_PCIe.sys: IOCTL_GET_DMA_READ_COUNT"));
		KdPrint(("   dma read count = %d\n", *(pBuffer)));
		break;
	...
	case IOCTL_GET_DMA_READ_PATTERN:
		...
		length = 1;
		*pBuffer = READ_REGISTER_ULONG ((PULONG) address);		
		KdPrint(("PM30_PCIe.sys: IOCTL_GET_DMA_READ_PATTERN"));
		KdPrint(("   dma read pattern = 0x%x\n", *(pBuffer)));
		break;
	...
	case IOCTL_GET_DMA_REGISTER:
		...
		length = 1;
		*pBuffer = READ_REGISTER_ULONG ((PULONG) address);
		KdPrint(("PM30_PCIe.sys: IOCTL_GET_DMA_REGISTER"));
	...
	case IOCTL_CLEANUP:
		...
		KdPrint(("PM30_PCIe.sys: IOCTL_CLEANUP\n"));
		length = 1;
		break;
	...
	case IOCTL_DISABLE_INTERRUPTS:
		...
		WRITE_REGISTER_ULONG ((PULONG) address, regValue);		// Write the new register value
		KdPrint(("PM30_PCIe.sys: IOCTL_DISABLE_INTERRUPTS\n"));
		break;
	...
	case IOCTL_ENABLE_INTERRUPTS:
		...
		WRITE_REGISTER_ULONG ((PULONG) address, regValue);		// Write the new register value
		KdPrint(("PM30_PCIe.sys: IOCTL_ENABLE_INTERRUPTS\n"));
		break;

   	default:
		// Set failure status for unrecognized control codes.
		KdPrint(("PM30_PCIe.sys: Received invalid I/O Control code.\n"));
  ...
  )
{
	...
	case IRP_MJ_CREATE:
		...
		KdPrint(("PM30_PCIe.sys: IRP_MJ_CREATE\n"));
	...
	case IRP_MJ_CLOSE:
		...
		KdPrint(("PM30_PCIe.sys IRP_MJ_CLOSE\n"));
	...
	case	IRP_MJ_READ:
		...
		KdPrint(("PM30_PCIe.sys: IRP_MJ_READ\n"));
	...
	case	IRP_MJ_WRITE:
		...
		KdPrint(("PM30_PCIe.sys: IRP_MJ_WRITE\n"));
	...
	default:	// Invalid 
	    KdPrint(("PM30_PCIe.sys IRP_MJ_NOT_IMPLEMENTED\n"));
		status = STATUS_NOT_IMPLEMENTED;
		Irp->IoStatus.Status = status;
		Irp->IoStatus.Information = 0;
		IoCompleteRequest(Irp, IO_NO_INCREMENT);
...
{    
	...
	KdPrint(("PM30_PCIe.sys: StartDMARead\n"));
		...
		!= STATUS_SUCCESS)
	{
			...
			KdPrint(("PM30_PCIe.sys: StartDMARead - GetScatterGatherList failed\n"));
	...
	KeLowerIrql(oldIrql);
	KdPrint(("PM30_PCIe.sys: StartDMARead completed\n"));
...
{    
	...
	KdPrint(("PM30_PCIe.sys: StartDMAWrite\n"));
		...
		!= STATUS_SUCCESS)
	{
			...
			KdPrint(("PM30_PCIe.sys: StartDMAWrite - GetScatterGatherList failed\n"));
	...
	KeLowerIrql(oldIrql);
    KdPrint(("PM30_PCIe.sys: StartDMAWrite completed\n"));
  ...
  IN PVOID pContext)
{
	...
	KdPrint(("PM30_PCIe.sys: ReadAdapterControl - BMDMA Read using System S/G\n"));
	...
	KdPrint(("PM30_PCIe.sys: ReadAdapterControl completed\n"));
	return;
  ...
  IN PVOID pContext)
{
	...
	KdPrint(("PM30_PCIe.sys: WriteAdapterControl\n"));
	...
	// We're done with the adapter channel object, but we must keep the map registers around for the 
	// device to use for the transfer.  The return code signals this intent to the I/O Manager.
	KdPrint(("PM30_PCIe.sys: WriteAdapterControl completed\n"));
	return;
  ...
  )
{
	...
	KdPrint (("PM30_PCIe.sys: DriverUnload\n"));
...
{							
    ...
    KdPrint(("PM30_PCIe.sys: CheckTimer\n"));
			...
			IN PVOID pServiceContext		) 
{
    ...
    KdPrint(("PM30_PCIe.sys: BDMA_Isr\n"));
	...
	KdPrint(("PM30_PCIe.sys: BDMA_Isr completed\n"));
	...
	)
{
	...
	KdPrint(("PM30_PCIe.sys: DpcForIsr\n"));
	...
	if (pDevExt->WriteLength > 0 )
	{
		...
		if (pDevExt->WriteOffset >= pDevExt->WriteTotalLength)
		{
			...
			KdPrint(("PM30_PCIe.sys: DpcForIsr - PCIe Write BMDMA Transfer Complete\n"));
			...
			if (!IsListEmpty(&pDevExt->WriteQueue))
			{
                ...
                KdPrint(("PM30_PCIe.sys: DpcForIsr - new write IRP started\n"));
		...
		{
			KdPrint(("PM30_PCIe.sys: DpcForIsr - more to do on this write transfer\n"));
	...
	if (pDevExt->ReadLength > 0)
	{
		...
		if (pDevExt->ReadOffset >= pDevExt->ReadTotalLength)
		{
			...
			KdPrint(("PM30_PCIe.sys: DpcForIsr - PCIe Read BMDMA Transfer Complete\n"));
			...
			if (!IsListEmpty(&pDevExt->ReadQueue))
			{
               ...
               KdPrint(("PM30_PCIe.sys: DpcForIsr - new read IRP started\n"));
		...
		{
			 KdPrint(("PM30_PCIe.sys: DpcForIsr - more to do on this read transfer\n"));
			(void) StartDMARead(pDevObj, pDevExt->ReadIrp);
		}
	}
	 KdPrint(("PM30_PCIe.sys: DpcForIsr completed\n"));
}
