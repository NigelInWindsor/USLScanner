  ...
  )
{
	...
	KdPrint (("PM30_PCIe.sys: AddDevice\n"));
  ...
  // this device driver.
	...
	{
		KdPrint (("PM30_PCIe.sys: IoCreateDevice exits with 0x%x\n", status));
		return status;
	}
    ...
    if (!NT_SUCCESS (status))  
	{
	  KdPrint (("PM30_PCIe.sys: IoRegisterDeviceInterface exits with 0x%x\n", status));
...
//    deviceObject->Flags |= DO_BUFFERED_IO;					// Buffered I/O
	...
	KdPrint (("PM30_PCIe.sys: AddDevice complete\n"));
	return status;
} // AddDevice
  ...
  )
{
	...
	KdPrint (("PM30_PCIe.sys:  DispatchPnP\n"));
	...
	{ // Someone has set the remove device lock
		KdPrint (("PM30_PCIe.sys: DispatchPnP exits - RemoveLock set\n"));
	...
	if (!NT_SUCCESS (status))  
	{
		KdPrint (("PM30_PCIe.sys: SendIrp exits with 0x%x\n", status));
	...
	case IRP_MN_START_DEVICE:
		...
		KdPrint(("PM30_PCIe.sys: IRP_MN_START_DEVICE\n"));
  ...
  )
{
	...
	LogMsg(DeviceObject, L"PnPStartDevice: PM30_PCIe PCIe driver device started");
	KdPrint (("PM30_PCIe.sys: PnPStartDevice\n"));
      ...
      ioStack->Parameters.StartDevice.AllocatedResourcesTranslated)  
	{
			 ...
			 i++, rawResource++, translatedResource++)  
		{
			...
			case CmResourceTypeNull:
				KdPrint (("PM30_PCIe.sys: Resource RAW Null\n"));
				break;

			case CmResourceTypePort:
				KdPrint (("PM30_PCIe.sys: Resource Raw Port: (0x%x) Length: (0x%x)\n",
				  rawResource->u.Port.Start.LowPart, rawResource->u.Port.Length));

				switch (translatedResource->Type)  
				{
				case CmResourceTypePort:
					KdPrint (("PM30_PCIe.sys: Resource Translated Port: (0x%x) Length: (0x%x)\n",
					  translatedResource->u.Port.Start.LowPart, 
					  translatedResource->u.Port.Length));
					break;

				case CmResourceTypeMemory:
					KdPrint (("PM30_PCIe.sys: Resource Translated Memory: (0x%x) Length: (0x%x)\n",
					  translatedResource->u.Memory.Start.LowPart, 
					  translatedResource->u.Memory.Length));
					break;

				default:
					KdPrint (("PM30_PCIe.sys: Unhandled resource 0x%x\n", translatedResource->Type));
					break;
				}
			...
			case CmResourceTypeInterrupt:
				KdPrint (("PM30_PCIe.sys: Resource Interrupt provided\n"));
			...
			case CmResourceTypeMemory:
				KdPrint (("PM30_PCIe.sys: Resource Raw Memory: (0x%x) Length: (0x%x)\n",
				...
				KdPrint (("PM30_PCIe.sys: Translated resource is memory at 0x%x\n", 
	...
	if (status != STATUS_SUCCESS)
	{
	    ...
	    KdPrint (("PM30_PCIe.sys: PnPStartDevice - IoConnectInterrupt failed\n"));
	}
	...
	else
	{
		...
		// Record the action in the Event Log.
	    LogMsg(DeviceObject, L"PnPStartDeviceDevice: PM30_PCIe PCIe driver Start Device completed");
	KdPrint (("PM30_PCIe.sys: PnPStartDevice completed\n"));
	}
  ...
  )
{
	...
	// Record the action in the Event Log.
	LogMsg(DeviceObject, L"PnPQueryStopDevice: PM30_PCIe PCIe driver received a query stop");
	KdPrint (("PM30_PCIe.sys: PnPQueryStopDevice\n"));
	...
	{
		// Record the action in the Event Log.
	    LogMsg(DeviceObject, L"PnPQueryStopDevice: PM30_PCIe PCIe driver was not started");
	    KdPrint (("PM30_PCIe.sys: PnPQueryStopDevice - device was not started\n"));
        IoReleaseRemoveLock(&deviceExtension->RemoveLock, Irp);
        status = DispatchClose(Irp, status, 0);
		return status;
	}
	...
	{
      // Record the action in the Event Log.
	  LogMsg(DeviceObject, L"PnPQueryStopDevice: PM30_PCIe PCIe driver - not OK to stop device");
	  KdPrint (("PM30_PCIe.sys: PnPQueryStopDevice - not OK to stop device\n"));
      IoReleaseRemoveLock(&deviceExtension->RemoveLock, Irp);
	...
	KdPrint(("PM30_PCIe.sys: To STOPPENDING from %s\n", 
	...
	{ // Someone has set the remove device lock
		KdPrint (("PM30_PCIe.sys: PnPQueryStopDevice exits - RemoveLock set\n"));
        IoReleaseRemoveLock(&deviceExtension->RemoveLock, Irp);
		status = DispatchClose(Irp, status, 0);
		return status;
	}
	...
	// Record the action in the Event Log.
	LogMsg(DeviceObject, L"PnPQueryStopDevice: PM30_PCIe PCIe driver - query stop completed");
	KdPrint (("PM30_PCIe.sys: PnPQueryStopDevice completed\n"));
  ...
  )
{
	...
	// Record the action in the Event Log.
	LogMsg(DeviceObject, L"PnPStopDevice: PM30_PCIe PCIe driver received a stop device");
	KdPrint (("PM30_PCIe.sys: PnPStopDevice\n"));
	...
	KdPrint(("PM30_PCIe.sys: To STOPPED from %s\n", 
		         statenames[deviceExtension->DevicePnPState]));
	SET_NEW_PNP_STATE (deviceExtension, Stopped);

	// Record the action in the Event Log.
	LogMsg(DeviceObject, L"PnPStopDevice: PM30_PCIe PCIe driver stopped");
	KdPrint (("PM30_PCIe.sys: PnPStopDevice complete\n"));
  ...
  )
{
	...
	// Record the action in the Event Log.
	LogMsg(DeviceObject, L"PnPCancelStopDevice: PM30_PCIe PCIe driver received a cancel stop");
	KdPrint (("PM30_PCIe.sys: PnPCancelStopDevice\n"));
	...
	{						// we succeeded earlier query
	    ...
	    {					// completed successfully
			KdPrint(("PM30_PCIe.sys: To STARTED from PENDINGSTOP\n"));
			...
			KdPrint(("PM30_PCIe.sys: Status %8.8lX returned by PDO for IRP_MN_CANCEL_STOP_DEVICE", status));

        IoReleaseRemoveLock(&deviceExtension->RemoveLock, Irp);		
		status = DispatchClose(Irp, status, 0);
        
		// Record the action in the Event Log.
	    LogMsg(DeviceObject, L"PnPCancelStopDevice: PM30_PCIe PCIe driver SendIrpSync failed");
		KdPrint (("PM30_PCIe.sys: PnPCancelStopDevice - SendIrpSync failed\n"));
        return status;
								// we succeeded earlier query
    }
	// Make sure the Remove Lock is not set prior to passing this IRP down. 
    ...
    if (&deviceExtension->RemoveLock && 
		status != IoAcquireRemoveLock(&deviceExtension->RemoveLock, Irp))
	{ // Someone has set the remove device lock
		KdPrint (("PM30_PCIe.sys: PnPQueryCancelStopDevice exits - RemoveLock set\n"));
        IoReleaseRemoveLock(&deviceExtension->RemoveLock, Irp);
		status = DispatchClose(Irp, status, 0);
		return status;
	}
	...
	KdPrint(("PM30_PCIe.sys: To %s from STOPPENDING in PnPCancelStopDevice\n", 
		         statenames[deviceExtension->DevicePnPState]));
	
	// Record the action in the Event Log.
	LogMsg(DeviceObject, L"PnPCancelStopDevice: PM30_PCIe PCIe driver stop canceled");
	KdPrint (("PM30_PCIe.sys: PnPCancelStopDevice complete\n"));
...
PnPQueryRemoveDevice(
	...
	{							// HandleQueryRemove
       ...
       // Record the action in the Event Log.
	   LogMsg(DeviceObject, L"PnPQueryRemoveDevice: PM30_PCIe PCIe driver received a query remove device");
	   KdPrint (("PM30_PCIe.sys: PnPQueryRemoveDevice\n"));
	   ...
	   {						// currently working
		  ...
		  {
			  // Record the action in the Event Log.
	          LogMsg(DeviceObject, L"PnPQueryRemoveDevice: PM30_PCIe PCIe driver - not OK to remove device");
	          KdPrint (("PM30_PCIe.sys: PnPQueryPRemoveDevice - not OK to remove device\n"));
		...
		KdPrint(("PM30_PCIe.sys: To PENDINGREMOVE from %s\n", 
		           statenames[deviceExtension->DevicePnPState]));
	   ...
	   // Record the action in the Event Log.
	   LogMsg(DeviceObject, L"PnPQueryRemoveDevice: PM30_PCIe PCIe driver remove device query received");
	   KdPrint (("PM30_PCIe.sys: PnPQueryRemoveDevice completed\n"));
  ...
  )
{
	...
	// Record the action in the Event Log.
	LogMsg(DeviceObject, L"PnPRemoveDevice: PM30_PCIe PCIe driver received a remove device");
	KdPrint (("PM30_PCIe.sys: PnPRemoveDevice\n"));
	
	status = PnPStopDevice(DeviceObject, Irp);

	deviceExtension = (PDEVICE_EXTENSION) DeviceObject->DeviceExtension;
    KdPrint(("PM30_PCIe.sys: To DELETED from %s\n", 
		         statenames[deviceExtension->DevicePnPState]));
    ...
    // Record the action in the Event Log.
	LogMsg(DeviceObject, L"PnPRemoveDevice: PM30_PCIe device removed sucessfully");
	KdPrint (("PM30_PCIe.sys: PnPRemoveDevice completed\n"));
  ...
  IN PIRP               Irp)
{
	...
	// Record the action in the Event Log.
	LogMsg(DeviceObject, L"PnPCancelRemoveDevice: PM30_PCIe PCIe driver received a cancel remove device");
	KdPrint (("PM30_PCIe.sys: PnPCancelRemoveDevice\n"));
	...
	{						// we succeeded earlier query
		...
		if (status == STATUS_SUCCESS)
		{					// completed successfully
			KdPrint(("PM30_PCIe.sys: To %s from PENDINGREMOVE\n", 
		...
		{
			KdPrint(("PM30_PCIe.sys: Status %8.8lX returned by PDO for IRP_MN_CANCEL_REMOVE_DEVICE", status));
		}
        RESTORE_PREVIOUS_PNP_STATE (deviceExtension);
	    KdPrint(("PM30_PCIe.sys: To %s from STOPPENDING in PnPCancelRemoveDevice\n", 
		         statenames[deviceExtension->DevicePnPState]));
        IoReleaseRemoveLock(&deviceExtension->RemoveLock, Irp);
		status = DispatchClose(Irp, status, 0);
	    return status;
    ...
    if (&deviceExtension->RemoveLock && 
		status != IoAcquireRemoveLock(&deviceExtension->RemoveLock, Irp))
	{ // Someone has set the remove device lock
		LogMsg(DeviceObject, L"PnPCancelRemoveDevice: PM30_PCIe device remove lock was set!");
	    KdPrint (("PM30_PCIe.sys: PnPQueryCancelStopDevice exits - RemoveLock set\n"));
        IoReleaseRemoveLock(&deviceExtension->RemoveLock, Irp);
	    status = DispatchClose(Irp, status, 0);
	    return status;
    ...
    LogMsg(DeviceObject, L"PnPCancelRemoveDevice: PM30_PCIe device remove canceled successfully!");
	KdPrint (("PM30_PCIe.sys: PnPCancelRemoveDevice completed\n"));
	IoReleaseRemoveLock(&deviceExtension->RemoveLock, Irp);
	status = DispatchClose(Irp, status, 0);
	return status;
  ...
  IN PIRP               Irp)
{
	...
	LogMsg(DeviceObject, L"PnPSurpriseRemoval: PM30_PCIe device surprise removal!");
	KdPrint (("PM30_PCIe.sys: PnPSurpriseRemoval\n"));
	...
	KdPrint(("PM30_PCIe.sys: To SURPRISEREMOVED from %s\n",statenames[deviceExtension->DevicePnPState]));
    ...
    if (&deviceExtension->RemoveLock && 
		status != IoAcquireRemoveLock(&deviceExtension->RemoveLock, Irp))
	{ // Someone has set the remove device lock
		LogMsg(DeviceObject, L"PnPSurpriseRemoval: PM30_PCIe device surprise removal - remove lock was set!");
	    KdPrint (("PM30_PCIe.sys: PnPSurpriseRemoval - remove lock was set!\n"));
        IoReleaseRemoveLock(&deviceExtension->RemoveLock, Irp);
		status = DispatchClose(Irp, status, 0);
		return status;
	}
	...
	LogMsg(DeviceObject, L"PnPSurpriseRemoval: PM30_PCIe device surprise removal completed");
	KdPrint (("PM30_PCIe.sys: PnPSurpriseRemoval complete\n"));
	IoReleaseRemoveLock(&deviceExtension->RemoveLock, Irp);
	status = DispatchClose(Irp, status, 0);
	return status;
} // PnPSurpriseRemoval
  ...
  )
{
	...
	KdPrint (("PM30_PCIe.sys: DispatchPower\n"));
	...
	KdPrint (("PM30_PCIe.sys: DispatchPower complete\n"));
