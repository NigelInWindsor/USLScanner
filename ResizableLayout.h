...
public:
	...
	public:
		...
		LayoutInfo(HWND hwnd, SIZE tl_t, SIZE tl_m, 
			SIZE br_t, SIZE br_m, BOOL hscroll, BOOL refresh, BOOL LockTL = FALSE, BOOL LockBR = FALSE)
		{
			hWnd = hwnd;
			nCallbackID = 0;
...
protected:
	...
	void AddAnchor(HWND hWnd, CSize sizeTypeTL, CSize sizeTypeBR = NOANCHOR,CSize sizeTypeOffset = CSize(0,0),BOOL LockTL = FALSE, BOOL LockBR = FALSE);

	// add anchors to a control, given its ID
	void AddAnchor(UINT nID, CSize sizeTypeTL, CSize sizeTypeBR = NOANCHOR,CSize sizeTypeOffset = CSize(0,0),BOOL LockTL = FALSE, BOOL LockBR = FALSE)
	{
		AddAnchor(::GetDlgItem(GetResizableWnd()->GetSafeHwnd(), nID),
			sizeTypeTL, sizeTypeBR, sizeTypeOffset,LockTL ,LockBR);
	}
